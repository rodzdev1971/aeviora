...
const phone = z
  ...
  .pipe(
    z
      ...
      .regex(
        /^\+[1-9]\d{7,14}$/,
        "Include the country code for international mobile numbers.",
      ),
  );
...
export function createRegistrationSchema(requiredAddressFields = []) {
  return z
    ...
    .superRefine((data, ctx) => {
      ...
      if (
        ["sms", "both"].includes(data.communicationPreference) &&
        !data.smsConsent
      ) {
        ctx.addIssue({
          code: "custom",
          path: ["smsConsent"],
          message: "SMS consent is required when choosing text messages.",
        });
      }
    });
}
